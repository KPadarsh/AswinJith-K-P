    ...
    <nav className="flex justify-between items-center px-8 md:px-24 py-8 md:py-12 relative z-50">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-[#fff5d9] shadow-lg py-8 flex flex-col items-center space-y-6 md:hidden z-50">
          ...
          <Link to="/contacts" onClick={toggleMenu} className={`text-lg hover:text-primary transition-colors ${location.pathname === '/contacts' ? 'border-b border-primary/40 pb-0.5 text-primary font-medium' : 'text-primary/70'}`}>Contacts</Link>
        </div>
      )}
    </nav>
  );
